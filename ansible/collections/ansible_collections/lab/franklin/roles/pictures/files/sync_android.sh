...
# v0.1 | 01/20/2023 | franklin@bitsmasher.net
# v0.2 | 10/18/2024 | franklin@bitsmasher.net
...
function create_target_dirs() {
  ...
  for td in "${TARGET_DIRS[@]}"; do
    if [ ! -d "${td}" ]; then
      mkdir -p "${td}"
      echo -e "${LGREEN}Created directory: ${td}${NC}\n"
    fi
  done
}
...
function copy_pictures() {
  ...
  eval ls "${BASE_DIR}/DCIM/Camera/*.jpg" | while read line; do
    ...
    echo -e "${LBLUE}File ${NC}${line}${LBLUE} was created on ${NC}${CREATE_DATE}"
  done
  ...
  eval cp -R "${BASE_DIR}/Pictures/*" /home/franklin/Pictures/Phone/Pictures

  # copy Internal storage/Download
  eval ls "${BASE_DIR}/Download/*"
  eval cp -R "${BASE_DIR}/Download/*" /home/franklin/Pictures/Phone/Download
...
function copy_movies() {
  ...
  if [ -d "${BASE_DIR}/DCIM/Videocaptures" ];
  then
...
function main() {
  echo -e "${LGREEN}Finding device: ${NC}"
  lsusb | grep Samsung
  echo -e "\n"
