...
class mash:
    ...
    def _extract(self):
        ...
        self.tempo["in"], self.beats["in"] = librosa.beat.beat_track(
            y=self.Yin, sr=self.sr
        )
        self.tempo["out"], self.beats["out"] = librosa.beat.beat_track(
            y=self.Yout, sr=self.sr
        )
    ...
    def _OTAC(self):  # Optimal Tempo Adjustment Coefficient Computation
        ...
        a, b = 0.765, 1
        Ttgt = (a - b) * Tlow + np.sqrt(
            ((a - b) ** 2) * (Tlow**2) + 4 * a * b * Thigh * Tlow
        )
        Ttgt = Ttgt / (2 * a)
    ...
    def _crossFadeRegion(self):  # Computes the cross fade region for the mixed song
        ...
        fadeInStart = librosa.frames_to_time(self.beats["in"], sr=self.sr)[
            -int(noBeats / 2)
        ]
        fadeIn = inDuration - fadeInStart

        fadeOut = librosa.frames_to_time(self.beats["out"], sr=self.sr)[
            int(noBeats / 2)
        ]
